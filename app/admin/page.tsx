...
export default function AdminPage() {
  ...
  const { data: configData } = useSWR(
    ...
    (key: string | [string, string]) =>
      Array.isArray(key) ? fetcher(key[0], key[1]) : fetcher(key),
    { refreshInterval: 2000 }
  );
          ...
          <div className="flex justify-between items-center border-b border-black pb-2">
            ...
            <span
              className={`text-[10px] uppercase font-bold px-1.5 py-0.5 border border-black ${
                config?.isTimerRunning ? "bg-[#fff5e2] text-black" : "bg-[#f5f5f5] text-[#6b7280]"
              }`}
            >
              {config?.isTimerRunning ? "RUNNING" : "PAUSED"}
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {config?.isTimerRunning ? (
              ...
              <button
                ...
                className="py-2 bg-[#A30B37] text-white border-2 border-black text-xs font-bold uppercase flex items-center justify-center space-x-1 hover:bg-[#85082c] shadow-[2px_2px_0px_#000]"
              >
